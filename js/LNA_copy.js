$(function () {
    ...
    $('#smith-input-parameters').off('submit').on('submit', function (e) {
        ...
        function main() {
            ...
            $('#Load-CL-Result').html('CL = ' + CL.re.toFixed(4) + (CL.im > 0 ? " + " : " - ") + "j" + Math.abs(CL.im).toFixed(4) + ' = ' + mCL.toFixed(4) + ' &ang; ' + argCL.toFixed(4) + '&deg;');
            $('#Load-RL-Result').html('RL = ' + RL.toFixed(4));
            $('#Load-CS-Result').html('CS = ' + CS.re.toFixed(4) + (CS.im > 0 ? " + " : " - ") + "j" + Math.abs(CS.im).toFixed(4) + ' = ' + mCS.toFixed(4) + ' &ang; ' + argCS.toFixed(4) + '&deg;');
            $('#Load-RS-Result').html('RS = ' + RS.toFixed(4));
            ...
            if (10 * Math.log10(GTerror_max) < 1) {
                $("#GT-error-result").append('<br> Đơn hướng');
                Dir = 1;
            } else {
                $("#GT-error-result").append('<br> Song hướng');
                Dir = 2;
            }
            if (Fdb) {
                ...
                $('#LNA2-result').html('\$C_F = ' + CF.abs().toFixed(4) + ' &ang; ' + (180 * CF.arg() / Math.PI).toFixed(4) + '&deg;\$');

                $('#LNA3').html('\$R_{F}=\\dfrac{\\sqrt{N(N+1-|\\Gamma_{\\mathrm{opt}}|^{2})}}{N+1}\$')
                $('#LNA3-result').html('\$R_F = ' + RF.toFixed(4) + '\$');
                ...
                if (Dir === 1) {
                    ...
                    $('#LNA5').html('\$\\Gamma_S = \$' + GSj.abs().toFixed(4) + ' &ang; ' + (180 * GSj.arg() / Math.PI).toFixed(4) + '&deg;');
                    ...
                    $('#LNA6-result').html('\$\\Gamma_{out} = \$' + Gout.abs().toFixed(4) + ' &ang; ' + (180 * Gout.arg() / Math.PI).toFixed(4) + '&deg;')

                    $('#LNA7').html('Để tối ưu hoá hệ số KĐCS thì chọn: \$\\Gamma_L = S_{22}^{*}\$')
                    $('#LNA7-result').html('Tính được: \$\\Gamma_L = \$' + GLj.abs().toFixed(4) + ' &ang; ' + (180 * GLj.arg() / Math.PI).toFixed(4) + '&deg;');
                    ...
                    $('#LNA6').html('\$c_{s}={\\dfrac{g_{A}C_{1}^{*}}{1+g_{_A}(\\left|S_{11}\\right|^{2}-\\left|\\Delta\\right|^{2})}}\$')
                    $('#LNA6-result').html('\$R_{s}=\\dfrac{\\sqrt{1-2K\\bigl|S_{12}S_{21}\\bigl|g_{A}+\\bigl|S_{12}S_{21}\\bigr|^{2}g_{A}^{2}}}{1+g_{A}\\bigl(|S_{11}\\bigr|^{2}-\\bigl|\\Delta\\bigr|^{2}\\bigr)}\$')

                    $('#LNA7').html('\$C_{1}=S_{11}-S_{22}^{*}\\Delta\$')

                    let mCga_left;
                    // let mCga_left = (C1.con()).abs() / (((S12.mul(S21)).abs() / (K - Math.sqrt(K ** 2 - 1))) + modS11 ** 2 - mDel ** 2);
                    let mCga_mid;
                    let mCga_right;
                    if (K >= 1) {
                        mCga_left = 0.05;
                    ...
                    let GScon = board.create('point', [GSj.re, -GSj.im], {name: '&Gamma;^*_S', size: 0.5})
                    $('#LNA8').html('Tính được giá trị: \$\\Gamma_S = \$' + GSj.abs().toFixed(4) + ' &ang; ' + (180 * GSj.arg() / Math.PI).toFixed(4) + '&deg;');
                    $('#LNA-title9').html('Xác định \$\\Gamma_{out}')
                    $('#LNA9').html('Xác định \$\\Gamma_{out}: \\Gamma_{o u t}=S_{22}+{\\dfrac{S_{12}S_{21}\\Gamma_{S}}{1-S_{11}\\Gamma_{S}}\}\$')
                    $('#LNA9-result').html('\$\\Gamma_{out} = \$' + Gout.abs().toFixed(4) + ' &ang; ' + (180 * Gout.arg() / Math.PI).toFixed(4) + '&deg;')

                    $('#LNA10').html('Để PHTK đầu ra thì: \$\\Gamma_L = \\Gamma_{out}^{*}\$')
                    $('#LNA10-result').html('Tính được: \$\\Gamma_L = \$' + GLj.abs().toFixed(4) + ' &ang; ' + (180 * GLj.arg() / Math.PI).toFixed(4) + '&deg;');
                    MathJax.typeset()
                    board.create('point', [GLj.re, GLj.im], {name: '&Gamma;_L', size: 0.5})
                ...
                $('#LNA3-result').html('Để PHTK đầu ra thì: \$\\Gamma_L = \\Gamma_{out}^{*}\$')
                ...
                $('#LNA4').html('Tính được các giá trị: \$\\Gamma_S = \$' + GSj.abs().toFixed(4) + ' &ang; ' + (180 * GSj.arg() / Math.PI).toFixed(4) + '&deg;');
                $('#LNA5').html('Để PHTK đầu ra thì: \$\\Gamma_L = \\Gamma_{out}^{*}\$')
                $('#LNA5-result').html('Tính được: \$\\Gamma_L = \$' + GLj.abs().toFixed(4) + ' &ang; ' + (180 * GLj.arg() / Math.PI).toFixed(4) + '&deg;');
            ...
            function PHTK(Gamma, type) {
                ...
                if (type === 'S') {
                    $("#d-sol1").html(`${ddd[0].toFixed(4)} &#955;&hArr;${(ddd[0] * 360).toFixed(4)}&deg;<br>${ddd[1].toFixed(4)} &#955;&hArr;${(ddd[1] * 360).toFixed(4)}&deg;`);
                    $("#l-sol1").html(`${xxx[0].toFixed(4)}&#955; (hở mạch)&hArr;${(xxx[0] * 360).toFixed(4)}&deg;<br>${xxx[1].toFixed(4)}&#955; (hở mạch)&hArr;${(xxx[1] * 360).toFixed(4)}&deg;`);
                }
                if (type === 'L') {
                    $('#d-sol2').html(`${ddd[0].toFixed(4)} &#955;&hArr;${(ddd[0] * 360).toFixed(4)}&deg;<br>${ddd[1].toFixed(4)} &#955;&hArr;${(ddd[1] * 360).toFixed(4)}&deg;`);
                    $('#l-sol2').html(`${xxx[0].toFixed(4)}&#955; (hở mạch)&hArr;${(xxx[0] * 360).toFixed(4)}&deg;<br>${xxx[1].toFixed(4)}&#955; (hở mạch)&hArr;${(xxx[1] * 360).toFixed(4)}&deg;`);
                }
            }
